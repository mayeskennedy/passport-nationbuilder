...
function Strategy(options, verify) {
  ...
  this._profileURL = options.profileURL || 'https://' + options.slug +
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  url = uri.format(url);
  this.get(url, accessToken, function(err, body, res) {
    var json;
 ...
 *  headers
 */
Strategy.prototype.get = function(url, accessToken, done) {
  var headers = {
    'Authorization': this._oauth2.buildAuthHeader(accessToken),
    'Accept': 'application/json',
    'Content-Type': 'application/json'
  };
  this._oauth2._request("GET", url, headers, "", null, done);
}
